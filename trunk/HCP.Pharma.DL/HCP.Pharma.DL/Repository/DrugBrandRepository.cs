...
namespace HCP.Pharma.DL.Repository
{
    public class DrugBrandRepository : IDrugBrandRepository
    {
        ...
        public async Task<List<DO_DrugReorderLevel>> GetDrugReorderLevel(int BusinessKey, int StoreCode)
        {
            try
            {
                using (eSyaEnterprise db = new eSyaEnterprise())
                {
                    ...
                    var DrugRo = result.Where(c => !query.Select(fc => fc.DrugCode).Contains(c.DrugCode)).ToList();

                    var final = DrugRo.Union(result1).OrderBy(x => x.DrugCode).ToList();
        ...
        public async Task<List<DO_DrugBinInfo>> GetDrugBinInfoList(int BusinessKey, int StoreCode)
        {
            try
            {
                using (eSyaEnterprise db = new eSyaEnterprise())
                {
                    var query = await db.GtEpdrbi.Where(x => x.BusinessKey == BusinessKey && x.StoreCode == StoreCode && x.ActiveStatus == true)
                        .Select(r => new DO_DrugReorderLevel
                        {
                            DrugCode = r.DrugCode,
                        }).ToListAsync();

                    var result1 = await db.GtEpdrbi.Where(x => x.BusinessKey == BusinessKey && x.StoreCode == StoreCode && x.ActiveStatus == true)
                        .Join(db.GtEpdrro,
                         x => new { x.BusinessKey, x.StoreCode,  x.DrugCode },
                         y => new { y.BusinessKey, y.StoreCode,  y.DrugCode},
                         (x, y) => new { x, y })
                         .Join(db.GtEpdrcd,
                         xy => xy.x.DrugCode,
                         z => z.DrugCode,
                         (xy, z) => new { xy, z })
                        .Select(r => new DO_DrugBinInfo
                        {
                            BusinessKey = r.xy.x.BusinessKey,
                            DrugCode = r.xy.x.DrugCode,
                            DrugBrand = r.z.DrugBrand,
                            StoreCode = r.xy.x.StoreCode,
                            MaximumStockLevel = r.xy.y.MaximumStockLevel,
                            ReorderLevel = r.xy.y.ReorderLevel,
                            SafetyStockLevel = r.xy.y.SafetyStockLevel,
                            MinimumStockLevel = r.xy.y.MinimumStockLevel,
                            BinInfo = r.xy.x.BinInfo,
                            ActiveStatus = r.xy.x.ActiveStatus
                        }).ToListAsync();

                    var result = await db.GtEpdrro.Where(x => x.BusinessKey == BusinessKey && x.StoreCode == StoreCode && x.ActiveStatus == true)
                        .Join(db.GtEpdrcd,
                         x => x.DrugCode,
                         y => y.DrugCode,
                         (x, y) => new { x, y })
                    ...
                    var DrugBinInfo = result.Where(c => !query.Select(fc => fc.DrugCode).Contains(c.DrugCode)).ToList();

                    var final = DrugBinInfo.Union(result1).OrderBy(x => x.DrugCode).ToList();
        ...
        public async Task<DO_ReturnParameter> InsertOrUpdateDrugBinInfo(List<DO_DrugBinInfo> obj)
        {
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        var is_DrugBinInfo = obj.Where(w => !String.IsNullOrEmpty(w.DrugBrand)).Count();
                        if (is_DrugBinInfo <= 0)
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "Please Enter Drug Bin Info Details." };
                        }

                        foreach (var sd in obj.Where(w => !String.IsNullOrEmpty(w.BinInfo)))
                        {
                            GtEpdrbi cs_sd = db.GtEpdrbi.Where(x => x.BusinessKey == sd.BusinessKey
                                            && x.StoreCode == sd.StoreCode && x.DrugCode == sd.DrugCode).FirstOrDefault();
                            if (cs_sd == null)
                            {
                                var o_cssd = new GtEpdrbi
                                {
                                    BusinessKey = sd.BusinessKey,
                                    StoreCode = sd.StoreCode,
                                    DrugCode = sd.DrugCode,
                                ...
                                db.GtEpdrbi.Add(o_cssd);
                            }
                            else
                            {
                                cs_sd.BinInfo = sd.BinInfo;
                                cs_sd.ActiveStatus = sd.ActiveStatus;
                                cs_sd.ModifiedBy = sd.UserID;
                                cs_sd.ModifiedOn = System.DateTime.Now;
                                cs_sd.ModifiedTerminal = sd.TerminalID;
                        ...
                        await db.SaveChangesAsync();
                        dbContext.Commit();
                        return new DO_ReturnParameter() { Status = true, Message = "Drug Bin Info Created Successfully." };
                    }
                    catch (DbUpdateException ex)
                    {
                        dbContext.Rollback();
        ...
        #endregion Drug Bin Info

        #region Drug Vendor Link

        public async Task<List<DO_DrugVendorLink>> GetDrugVendorLink(int VendorCode)
        {
            try
            {
                using (var db = new eSyaEnterprise())
                {
                    var result = await db.GtEavndr
                             .Join(db.GtEpdrcd,
                             t => new { t.DrugCode },
                             b => new { b.DrugCode },
                             (t, b) => new { t, b })
                         .GroupJoin(db.GtEavncd,
                        tb => new { tb.t.VendorCode },
                        d => new { d.VendorCode },
                        (tb, d) => new { tb, d = d.FirstOrDefault() })
                         .Where(w => w.tb.t.VendorCode == VendorCode)
                        .Select(a => new DO_DrugVendorLink
                        {
                            DrugCode = a.tb.t.DrugCode,
                            DrugBrand = a.tb.b.DrugBrand,
                            BusinessShare = a.tb.t.BusinessShare,
                            MinimumSupplyQty = a.tb.t.MinimumSupplyQty,
                            RateType = a.tb.t.RateType,
                            RateTypeDesc = a.tb.t.RateType == "PO"?"Purchase Order":"Rate Type",
                            Rate = a.tb.t.Rate,
                            ActiveStatus = a.tb.t.ActiveStatus
                        }).ToListAsync();
                    return result;
        ...
        public async Task<DO_ReturnParameter> InsertOrUpdateVendorDrugLink(DO_DrugVendorLink obj)
        {
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    ...
                    {
                        int isInsert = 0;
                        GtEavndr sPar = db.GtEavndr.Where(x => x.VendorCode == obj.VendorCode && x.DrugCode == obj.DrugCode).FirstOrDefault();
                        if (sPar != null)
                        {
                            sPar.BusinessShare = obj.BusinessShare;
                            sPar.MinimumSupplyQty = obj.MinimumSupplyQty;
                            sPar.RateType = obj.RateType;
                            sPar.Rate = obj.Rate;
                            sPar.ActiveStatus = obj.ActiveStatus;
                            sPar.ModifiedBy = obj.UserID;
                            sPar.ModifiedOn = System.DateTime.Now;
                        ...
                        {
                            var strbusslnk = new GtEavndr
                            {
                                VendorCode = obj.VendorCode,
                                DrugCode = obj.DrugCode,
                                BusinessShare = obj.BusinessShare,
                                MinimumSupplyQty = obj.MinimumSupplyQty,
                        ...
                        if (isInsert == 1)
                            return new DO_ReturnParameter() { Status = true, Message = "Vendor Drug Link Created Successfully." };
                        else
                            return new DO_ReturnParameter() { Status = true, Message = "Vendor Drug Link Updated Successfully." };
                    }
                    catch (DbUpdateException ex)
                    {
                        dbContext.Rollback();

                ...
                using (var db = new eSyaEnterprise())
                {
                    var result = await db.GtEpdrgn
                        ...
                        //(t, b) => new { t, b=b.FirstOrDefault() })
                         .Join(db.GtEcapcd,
                        tb => new { ApplicationCode = tb.PharmacyGroup },
                        d => new { d.ApplicationCode },
                        (tb, d) => new { tb, d })
                         
                        .Select(a => new DO_DrugCategory
                        {
                            GenericId = a.tb.GenericId,
                            GenericDesc = a.tb.GenericDesc,
                            IsCombiDrug = a.tb.IsCombiDrug,
                            PharmacyGroup = a.tb.PharmacyGroup,
                            PharmacyGroupDesc = a.d.CodeDesc,
                            UsageStatus = a.tb.UsageStatus,
                            ActiveStatus = a.tb.ActiveStatus
                        }).ToListAsync();
                    return result;

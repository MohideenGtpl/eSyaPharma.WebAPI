...
namespace HCP.Pharma.DL.Repository
{
    public class GenericRepository : IGenericRepository
    {
        ...
        public async Task<List<DO_Generic>> GetGenericByPrefix(string prefix)
        {
            try
            {
                    ...
                    var ds = _context.GtEpdrgn
                        ...
                        .Where(w => (w.gcp.gc.g.GenericDesc.ToUpper().StartsWith(prefix.ToUpper()) || prefix == "All"))
        ...
        public async Task<List<DO_Composition>> GetCompositionByPrefix(string prefix)
        {
            try
            {
                ...
                var ds = _context.GtEpdrco
                    ...
                    .Where(w => (w.gc.g.CompositionDesc.ToUpper().StartsWith(prefix.ToUpper()) || prefix == "All"))
        ...
        public async Task<List<DO_GenericComposition>> GetGenericComposition(string prefix)
        {
            try
            {

                var ds = _context.GtEpdrgc
                    .GroupJoin(_context.GtEpdrgn,
                      l => l.GenericId,
                      g => g.GenericId,
                      (l, g) => new { l, g = g.FirstOrDefault() }
                      )
                    .GroupJoin(_context.GtEpdrco,
                      lg => lg.l.CompositionId,
                      c => c.CompositionId,
                      (lg, c) => new { lg, c = c.FirstOrDefault() }
                      )
                      .Where(w => w.lg.g.GenericDesc.ToUpper().StartsWith(prefix.ToUpper()) || prefix == "All")
                    .Select(r => new DO_GenericComposition
                    {
                        GenericId = r.lg.l.GenericId,
                        GenericDesc = r.lg.g.GenericDesc,
                        CompositionId=r.lg.l.CompositionId,
                        CompositionDesc=r.c.CompositionDesc,
                        ActiveStatus = r.lg.l.ActiveStatus,
                    }).OrderBy(o => o.GenericDesc).ToListAsync();
        ...
        public async Task<DO_GenericComposition> GetGenericCompositionByID(int GenericID,int CompositionID)
        {
            try
            {

                var ds = _context.GtEpdrgc
                    .Where(w => w.GenericId == GenericID && w.CompositionId == CompositionID)
                    .Select(r => new DO_GenericComposition
                    {
        ...
        public async Task<DO_ReturnParameter> AddOrUpdateGenericComposition(DO_GenericComposition obj)
        {
            var dbContext = _context.Database.BeginTransaction();
            try
            {
                    var _genericcomposition = _context.GtEpdrgc.Where(w => w.GenericId == obj.GenericId && w.CompositionId==obj.CompositionId).FirstOrDefault();
                    if (_genericcomposition == null)
                    {
                    var _newrecord = new GtEpdrgc()
                    {
                    ...
                    _context.GtEpdrgc.Add(_newrecord);
                }
                    else
                    {
                        _genericcomposition.ActiveStatus = obj.ActiveStatus;
                        _genericcomposition.ModifiedBy = obj.UserID;
                        _genericcomposition.ModifiedOn = DateTime.Now;
                        _genericcomposition.ModifiedTerminal = obj.TerminalID;
                    }
